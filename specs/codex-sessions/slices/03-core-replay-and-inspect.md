...
Status: complete on `codex-slice3`, pending integration. The model, timeline,
info, replay, live snapshot, browser single-file replay, and inspect path now
consume provider-neutral events. Claude wire compatibility exists only inside
tests and routes through the real Claude adapter; production consumers have no
wire-type bridge.

Implementation choices: snapshots transfer both byte identity/offset state and
each file's mutated decoder into live tailing. `WatchTarget` remains intact
through reattach so cwd watches do not become pinned files. Lifecycle facts are
reduced by event timestamp with a deterministic terminal tie-break, and
timestamped `interacted` facts count as child activity. Tool completion carries
neutral spawn-completion eligibility: Claude's legacy synchronous result may
complete a child, while Codex results cannot substitute for typed lifecycle.
Malformed Claude sidecars remain pending for live retry. Spawn provenance uses
an explicit evidence-strength merge so call-site context wins independent of
file arrival order. Equal-timestamp facts share one scrubber bin so
normalization granularity does not change the visible burst count.

Verified on Rust 1.88: 221 library and 8 binary native tests, 173 portable-core
tests, strict clippy, rustdoc warnings-as-errors, Claude inspect parity, and
explicit Codex root/child inspect smoke checks.

```yaml
review:
  pass_id: codex-sessions-03
  risk: high
  lane: subagent
...
The initial review found three snapshot/reducer correctness issues. Its first
directed recheck confirmed those fixes and found one transient-read retry gap;
the same reviewer confirmed that final fix in a second directed recheck and
